...
The coral-cover data behind the TCRMP, VINPS, and CSUN analyses comes from the Reef Code benthic cover section. The species-level file backs the TCRMP and VINPS analyses: [coral species cover download]({{< var data-source-species >}}). The genus-level file backs the CSUN analysis: [coral genera cover download]({{< var data-source-genera >}}).
...
The **as published** version reruns the analysis exactly as it appears in the manuscript. TCRMP and VINPS clip to 2001 through 2023. CSUN clips to 1992 through 2023. The octocoral program clips to 2014 through 2024.

The **updated since publication** version reruns the identical analysis: the same code, the same site scoping, and the same taxon scoping. Only the year range changes. Comparing the two versions side by side shows whether the pattern reported in the paper is holding, strengthening, or shifting as the monitoring record grows.
...
![Long-term coral monitoring sites across the US Virgin Islands from three independent programs: Territorial Coral Reef Monitoring Program (TCRMP, n = 20 sites), Virgin Islands National Park (VINPS, n = 7 sites), and California State University Northridge (CSUN, n = 8 sites). All sites were established before 2006 and are located at 0–20 m depth.](figures/Catalina_Map_300.png){#fig-map width=85%}
...
```{r tbl-si-sites}
#| echo: false
#| tbl-cap: "Site characteristics for the three long term monitoring programs. This table consolidates survey site information for the TCRMP, VINPS, and CSUN programs."
if (have_asp) {
...
#| echo: false
#| tbl-cap: "Taxonomic classification of corals analyzed from three monitoring programs. Shown are all observed  coral taxa (species or genera) observed in any monitoring program, indicating their reproductive mode (brooder or broadcaster) and commonness status (common or rare) within each program. For TCRMP and VINPS (species-level analyses), a '-' indicates the species was not observed/recorded in that program. For CSUN (genus-level analyses), a '-' indicates the genus was not observed/recorded. Blank cells indicate that the taxon was not part of that program's taxonomic resolution. The reproductive mode of genus- and species- level taxa represent the classification used for CSUN or TCRMP/VINPS, respectively."
if (have_asp) {
