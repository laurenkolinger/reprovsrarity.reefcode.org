...
The coral-cover data behind the TCRMP, VINPS, and CSUN analyses comes from the Reef Code benthic cover section. A species-level file backs TCRMP and VINPS ([coral species cover download]({{< var data-source-species >}})); a genus-level file backs CSUN ([coral genera cover download]({{< var data-source-genera >}})).

### Two versions of each analysis

Each analysis appears in two versions. Open a program from the Outputs menu to read the as submitted version, then use the toggle at the top of the page to switch to the updated version and back (as of July 2026, the two are the same).
