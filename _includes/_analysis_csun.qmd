...
- Analysis code: {{< downloadthis ../../_includes/_analysis_csun.qmd dname="analysis_csun.qmd" label="Download the analysis code (.qmd)" >}}

```{r downloads-csun}
#| echo: false
#| output: asis
.prog_key <- if (tolower(program) == "octocoral") "octo" else tolower(program)
.files <- sort(list.files("../../data/derived", pattern = paste0("^pub_rarity_", .prog_key, "_.*\\.(csv|txt)$")))
if (length(.files)) {
  cat("| File | Type | Download |\n|:--|:--|:--|\n")
  for (f in .files) {
    lbl <- sub(paste0("^pub_rarity_", .prog_key, "_"), "", f)
